    ...
    <div className="flex flex-col relative bg-main-white overflow-auto shadow-md rounded-small w-full">
      <table className={cn("w-full text-sm text-right")}>
        <thead className=" text-heading h-fit">
          <tr
            className={`grid grid-cols-${columns.length} gap-medium bg-cta-hover-secondary`}
          >
            {columns.map((column, index) => (
              <th
                key={index}
                className={` p-medium ${column.headerClassName || ""}`}
              >
        ...
        <tbody>
          {currentItems.map((item, rowIndex) => (
            <tr
              key={rowIndex}
              className={`grid grid-cols-${columns.length} gap-medium border-b-1 border-primary items-center`}
            >
